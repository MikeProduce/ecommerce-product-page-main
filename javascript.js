...
addCart.addEventListener("click", function () {
    ...
    if (quantityItems > 0) {
      ...
      <img class="shopimg" width="50px" height="50px" src="${item.image}">
      <div>
        <h5>${item.name}</h5>
        <h6>$${item.price} x ${quantityItems} $${
            item.price * quantityItems
        }</h6>
      </div>
      <div>
    ...
    <button class="addcart">Checkout</button>`;
...
// miguel you left off trying to make the modal first in mobile. you were about to add the checkout button and trying to decide how to display this. if we should use li or something else
